      ...
      <ul className="divide-y divide-border">
        {[
          ...
          <li key={r.label} className="flex items-center justify-between py-4">
            ...
            <Button variant="outline" size="sm" className="rounded-lg border-border bg-card hover:bg-beige">
              Manage
            </Button>
          </li>
        ))}
  ...
  return (
    <>
      <Card title="Gemini" desc="Model used for summaries, categorisation, and chat answers.">
        <RadioRow value={gemini} onChange={setGemini} options={[
          { id: "gemini-3-flash-preview", label: "Gemini 3 Flash (Preview)", hint: "Fast, low latency" },
      ...
      <Card title="NVIDIA NIM" desc="Used for high-throughput embeddings and structured extraction.">
        <RadioRow value={nim} onChange={setNim} options={[
...
function SyncSection() {
  return (
    <Card title="Sync preferences" desc="Control how Repeatless mirrors your Gmail.">
      <ToggleRow label="Initial historical sync" hint="Backfill all email history (one-time)." defaultOn />
      ...
      <ToggleRow label="Auto-categorize on arrival" hint="Run categorisation as emails sync." defaultOn />
    </Card>
  );
}
...
function NotificationsSection() {
  return (
    <Card title="Alerts & digests" desc="Choose what's worth interrupting you for.">
      <ToggleRow label="Priority email alerts" hint="Notify when an important email arrives." defaultOn />
      <ToggleRow label="Daily morning digest" hint="9:00 AM brief of overnight activity." defaultOn />
      <ToggleRow label="Weekly intelligence report" hint="Mondays at 8:00 AM." />
      <ToggleRow label="Newsletter digest" hint="Unified news extracted from your subscriptions." defaultOn />
    </Card>
  );
}
...
function GoogleMark(props: React.SVGProps<SVGSVGElement>) {
  return (
    <svg viewBox="0 0 48 48" {...props}>
      ...
      <path fill="#4CAF50" d="M24 44c5.2 0 9.9-2 13.4-5.2l-6.2-5.2C29.2 35 26.7 36 24 36c-5.3 0-9.7-3.1-11.3-7.5l-6.6 5.1C9.6 39.7 16.3 44 24 44z" />
      <path fill="#1976D2" d="M43.6 20.5H42V20H24v8h11.3c-.8 2.3-2.3 4.3-4.3 5.6l6.2 5.2C40.1 36 44 30.5 44 24c0-1.2-.1-2.3-.4-3.5z" />
    </svg>
  );
}