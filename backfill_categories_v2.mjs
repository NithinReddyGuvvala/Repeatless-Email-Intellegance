 ...
 * 
 * Usage: node backfill_categories_v2.mjs
 */
...
async function main() {
  ...
  if (alreadyCategorized >= totalEmails) {
    console.log("✅ All emails already categorized. Nothing to do.");
    return;
  }

  // 2. Find already-categorized email IDs
  ...
  while (true) {
    ...
    process.stdout.write(`\r  Progress: ${processed}/${totalEmails} processed, ${inserted} newly categorized, ${errors} errors`);
  ...
  if (finalCount >= (totalEmails * 0.99)) {
    console.log("\n✅ Categorization complete! Dashboard 'Categorized' count will now be correct.");
  } else {
